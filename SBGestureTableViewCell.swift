...
class SBGestureTableViewCell: UITableViewCell, UIGestureRecognizerDelegate {
    ...
    var rightAction: SBGestureTableViewCellAction? {
        didSet {
            if (rightAction?.fraction == 0) {
                //firstLeftAction?.fraction = 0.3
                rightAction?.fraction = 0.3
            }
        }
    }
//
...
//        }
        ...
        if frame.origin.x < 0 {
            if rightAction != nil && fraction > rightAction!.fraction {
                return rightAction!
            }
        }
        return nil
...
//        }
        if let action = action {
            ...
            if frame.origin.x > 0 {
                ...
                rightSideView.backgroundColor = actionNormalColor
                //rightSideView.iconImageView.image = firstRightAction!.icon
                rightSideView.iconImageView.image = rightAction!.icon
            }
